...
const TEACHERS = {
    lourenco: { userId: 'vUoy5malhvQe8RLSPBnzR24Vgh72', name: 'LOURENÇO FERREIRA DA SILVA NETTO' },
...
async function getCollaborators(classNum, examType) {
    ...
    if (examType === 'Linguagens') {
        ...
        if (classNum.startsWith('82') || classNum.startsWith('92')) {
            ...
            list.push({ ...TEACHERS.lourenco, subject: 'Educação Física', quota: 5, current: 0 });
        }
...
async function main() {
    console.log("Starting 3rd Bimester Exams Population...");

    const examsRef = db.collection('exams');
    
    console.log("Fetching all exams to archive 2nd bimester...");
    const allExamsSnapshot = await examsRef.get();
    let archivedCount = 0;
    
    console.log(`Found ${allExamsSnapshot.size} total exams in db.`);
    for (const doc of allExamsSnapshot.docs) {
        const data = doc.data();
        if ((data.bimester === '2º Bimestre' || data.bimester === '2 Bimestre' || (data.bimester && data.bimester.includes('2'))) && data.status === 'published') {
            await doc.ref.update({ status: 'archived' });
            archivedCount++;
        }
    }
    console.log(`Archived ${archivedCount} exams from 2nd Bimester.`);
    ...
    for (const classDoc of classesSnapshot.docs) {
        ...
        const classNum = className.replace("Turma ", "").trim(); 
        ...
        for (const type of examTypes) {
            ...
            const examData = {
                title: `${className} - 3º Bimestre - ${type.name}`,
    ...
    console.log(`\nSuccess! Created ${createdCount} exams in total for 3rd Bimester.`);
}
